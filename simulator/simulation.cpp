...
std::string trace_file = "traces/cluster001";
...
std::vector<size_t> remove_pages(int max_evict, int policy_no) {
  ...
  if (policy_no == 2) {
    sorted_client = utils.flip_map(client_map);
  } else if (policy_no == 3) {
    sorted_key = utils.flip_map(key_map);
  }
...
int test_policy(int policy_no) {
  ...
  std::ofstream perf;
  std::string curr_dir(std::getenv("PWD"));
  std::string dir(CSV_DIR);
  trace.open(trace_file);
  std::string file_type = ".csv";
  std::string pin_csv = curr_dir + dir + "pinning" + std::to_string(policy_no) + file_type;
  std::string perf_csv = curr_dir+ dir + "perf" + std::to_string(policy_no) + file_type;
  std::cout << "=============================================== CSV: " << pin_csv << "=================================================" << std::endl;
  pinning.open(pin_csv);
  perf.open(perf_csv);
  std::string line;
